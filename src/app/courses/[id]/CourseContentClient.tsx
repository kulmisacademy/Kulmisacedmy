...
export function CourseContentClient({ courseId }: Props) {
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          <p className="text-gray-600">Course not found or failed to load.</p>
          <Link href="/courses" className="mt-4 inline-block text-primary-600 hover:underline">
            ← Back to courses
          </Link>
        </div>
      </div>
  ...
  const { course, lessons, isEnrolled, userPhone, hasPendingPayment, session, reviews, resources } = data;
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6 sm:py-8 sm:px-6 lg:px-8">
            ...
            <div className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-[1fr,2fr] p-4 sm:p-6 lg:p-8">
              <div>
                <CourseThumbnail
                  ...
                  subtitle={
                    course.description
                      ? course.description.slice(0, 100).trim() + (course.description.length > 100 ? "…" : "")
                      : undefined
                  }
                />
                <p className="mt-4 text-2xl font-bold text-gray-900">{formatPrice(course.price)}</p>
                {((messageParam === "enroll" && !isEnrolled) ||
                  pendingParam === "1" ||
              ...
              <div>
                ...
                {course.description && <p className="mt-4 text-gray-600">{course.description}</p>}
                    ...
                    <li key={lesson.id}>
                      {canAccess ? (
                        ...
                        >
                          ...
                          <span
                            className="shrink-0 flex h-10 w-10 items-center justify-center rounded-full bg-gray-200 text-gray-400"
          ...
          {isEnrolled && Array.isArray(resources) && resources.length > 0 && (
            <CourseResourcesBlock resources={resources as CourseResource[]} />
          )}
          ...
          <CourseReviewsSection reviews={reviews as Parameters<typeof CourseReviewsSection>[0]["reviews"]} />
    </div>
  );
}
